...
clean:
	...
	$(Q)rm -f $(ANCIBLE_PLAYBOOK) $(TEST_CLI) $(TEST_ARGS) $(TEST_PARSER) $(TEST_INVENTORY) \
	          $(TEST_CONTEXT) $(TEST_RUNNER) $(TEST_SSH) $(TEST_COMMAND) \
	          $(TEST_COMMAND_MODULE) $(TEST_FILE_MODULE) $(TEST_COPY_MODULE) $(TEST_EXECUTOR) $(TEST_STATE) \
...
$(TEST_COPY_MODULE): $(TEST_DIR)/test_copy_module.c $(MODULES_DIR)/copy.o $(MODULES_DIR)/module.o $(TRANSPORT_DIR)/runner.o $(TRANSPORT_DIR)/ssh.o $(CORE_DIR)/context.o
	$(Q)printf " %s\n" "$(quiet_cmd_link)"
	$(Q)$(cmd_link)
